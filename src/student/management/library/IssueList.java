...
public class IssueList extends JFrame
{
    ...
    IssueList()
    {
        ...
        l1 = JFrameSize.headline("LIBRARY BOOKS");
        l1.setBounds(230,20,400,30);
        admin.add(l1);
        ...
        l2 = JFrameSize.title("Issued Book List");
        l2.setBounds(30,80,300,30);
        admin.add(l2);

        model.setDataVector(new Object[][]{{"1","2","3","4","5","6","EDIT","DELETE"},{"1","2","3","4","5","6","EDIT","DELETE"}},
                new Object[]{"Book No","Book Name","Status","Roll No","Name","RETURN"});

        table = new JTable(model);
        table.getColumn("RETURN").setCellRenderer(new ButtonRender());

        table.getColumn("RETURN").setCellEditor(new ButtonEditor(new JCheckBox(),this));
        ...
        admin.add(b2);
        b2.addActionListener(e ->{new Library(); Common.close(this);});
        ...
        select.addListSelectionListener(e->{
                ...
                for (int i = 0; i < row.length; i++) {
                    Data = (String) table.getValueAt(row[i], 4);
                }
    ...
    {
        String column[] = {"bookno", "bname", "status","rollno","name"};
        ArrayList list ;
        String str = "issued";
        ...
        try (Connection con = MyConnect.getInstance().getConnection()) {
            ...
            while(rs.next())
            {
                if(str.equalsIgnoreCase(rs.getString(3)))
                {
                    list= new ArrayList();
                    index=1;
